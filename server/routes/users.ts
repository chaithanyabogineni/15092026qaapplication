...
import { emailTemplate, inviteEmailTemplate, escapeHtml, isPrivateOrInternalUrl } from "../utils/helpers.ts";
...
router.post("/api/invite", async (req, res) => {
  ...
  try {
    ...
    if (!gmailUser || !gmailPass) {
      console.warn("[Invite API] Gmail SMTP credentials not configured.");
      return res.status(200).json({
        success: true,
        message: "Invite generated, but SMTP is unconfigured. Share the invite URL directly with the user.",
        inviteUrl
      });
    }
    ...
    const mailOptions = {
      from: `"Zeta QA Platform" <${gmailUser}>`,
      to: email,
      ...
      html: inviteEmailTemplate({
        ...
        role: role || "Viewer",
        team: team || "Zeta QA",
        inviteUrl: inviteUrl || `${req.headers.origin || ""}/signup`
      }),
    };
