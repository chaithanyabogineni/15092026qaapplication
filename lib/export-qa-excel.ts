...
const DEFAULT_FALLBACK_CHECKLISTS = [
...
/**
 * Builds an XLSX workbook object from the campaign checklist data
 * Organized STAGE-WISE with clear section headers, bold titles, and QA Type specification.
 */
export function buildQAChecklistWorkbook(params: ExportQAExcelParams): XLSX.WorkBook {
  const {
    campaignName = "Untitled Campaign",
    team = "HP-APJ",
    country = "Global",
    versionName = "v1",
    userEmail = "qa-lead@zetaglobal.com",
  ...
  // Use provided checklists or fallback to standard checkpoints
  const activeChecklists = (checklists && checklists.length > 0)
    ? checklists
    : DEFAULT_FALLBACK_CHECKLISTS;
  ...
  dataAOA.push([`ZETA GLOBAL QUALITY ASSURANCE - ${normalizedQaType} VERIFICATION REPORT`, "", "", "", "", ""]);

  // Row 1: Subtitle
  dataAOA.push(["Zeta Global Enterprise QA Platform Verification System", "", "", "", "", ""]);
...
/**
 * Exports and downloads the QA Checklist directly as an Excel file (.xlsx)
 */
export function exportQAChecklistToExcel(params: ExportQAExcelParams): void {
  const wb = buildQAChecklistWorkbook(params);
  const cleanName = (params.campaignName || "Campaign").replace(/[^a-zA-Z0-9_-]/g, "_");
  const qaTypeStr = (params.qaType || "CQA").replace(/[^a-zA-Z0-9_-]/g, "_");
  const filename = `ZETA_QA_Checklist_${qaTypeStr}_${cleanName}_${new Date().toISOString().slice(0, 10)}.xlsx`;
  XLSX.writeFile(wb, filename);
}

/**
 * Generates base64 string of the Excel file (.xlsx) to send as an email attachment
 */
export function generateQAChecklistExcelBase64(params: ExportQAExcelParams): { filename: string; base64: string } {
  const wb = buildQAChecklistWorkbook(params);
  const cleanName = (params.campaignName || "Campaign").replace(/[^a-zA-Z0-9_-]/g, "_");
  const qaTypeStr = (params.qaType || "CQA").replace(/[^a-zA-Z0-9_-]/g, "_");
  const filename = `ZETA_QA_Checklist_${qaTypeStr}_${cleanName}_${new Date().toISOString().slice(0, 10)}.xlsx`;
